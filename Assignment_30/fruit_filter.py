...
def add_filter(image, lips, right_eye, left_eye):
    lips = cv2.resize(lips, (500, 150))
    removed_background_lips = remove_background(lips)
    image = overlayPNG(image, removed_background_lips, pos=(750, 650))

    right_eye = cv2.resize(right_eye, (200, 150))
    removed_background_right_eye = remove_background(right_eye)
    image = overlayPNG(image, removed_background_right_eye, pos=(1100, 400))

    left_eye = cv2.resize(left_eye, (200, 150))
    removed_background_left_eye = remove_background(left_eye)
    image = overlayPNG(image, removed_background_left_eye, pos=(700, 400))
...
resized_fruit_image = cv2.resize(fruit_image, (image.shape[1], image.shape[0]))
...
result = add_filter(resized_fruit_image, lips,
                    right_eye, left_eye)
